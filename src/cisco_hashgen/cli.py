#!/usr/bin/env python3
from cisco_hashgen.cli import main
if __name__ == "__main__":
    main()
"""
Cisco HashGen v1.1
Generate and verify Cisco-compatible PBKDF2 password hashes.

Supports:
- ASA PBKDF2-HMAC-SHA512 ($sha512$...$...$...)
- IOS/IOS-XE Type 8 PBKDF2-HMAC-SHA256 ($8$...$...)
...
# Build help description (with optional colors)
def build_description(use_color):
    title = colorize("Cisco HashGen v1.1 — Generate and verify Cisco-compatible PBKDF2 hashes", "bold", "cyan", use_color=use_color)
    defaults_hdr = colorize("Defaults:", "bold", "green", use_color=use_color)
    quoting_hdr  = colorize("Quoting Guide for -pwd:", "bold", "blue", use_color=use_color)
    return (
        f"{title}\n"
        f"{defaults_hdr}\n"
        f"  {colorize('ASA PBKDF2-SHA512', 'yellow', use_color=use_color)}: iterations={ASA_DEFAULT_ITER}, salt-bytes={ASA_DEFAULT_SALT}\n"
        f"  {colorize('IOS/IOS-XE Type 8 PBKDF2-SHA256', 'yellow', use_color=use_color)}: iterations={IOS8_DEFAULT_ITER}, salt-bytes={IOS8_DEFAULT_SALT}\n"
        f"  Validation: minlen={MINLEN_DEFAULT}, maxlen={MAXLEN_DEFAULT}\n\n"
        f"{quoting_hdr}\n"
        "  POSIX shells (bash/zsh):\n"
        "    Use single quotes for literal strings:  -pwd 'pa ss $p!\"'\n"
        "    If password contains a single quote:    -pwd 'pa'\"'\"'ss'\n"
        "  PowerShell:\n"
        "    Use single quotes:                      -pwd 'pa ss $p!\"'\n"
        "    Single quote inside password:           -pwd 'it''s$weird'\n"
        "  Automation-safe:\n"
...
# ---------- Hash builders ----------
def build_asa_pbkdf2_sha512(password, iterations=ASA_DEFAULT_ITER, salt_len=ASA_DEFAULT_SALT):
...
def build_ios_type8(password, iterations=IOS8_DEFAULT_ITER, salt_len=IOS8_DEFAULT_SALT):
    salt = os.urandom(salt_len)  # IOS uses 10 bytes; we default to that
    dk = hashlib.pbkdf2_hmac("sha256", password, salt, iterations)
    return f"$8${_cisco_b64(salt)}${_cisco_b64(dk)}"

# ---------- Verification ----------
def verify_password(candidate, hash_str):
    if hash_str.startswith("$sha512$"):
        parts = hash_str.split("$")
        if len(parts) != 5: raise ValueError("Malformed ASA hash.")
        ...
        parts = hash_str.split("$")
        if len(parts) != 4: raise ValueError("Malformed IOS Type 8 hash.")
        salt = _cisco_b64_decode(parts[2])
        dk_stored = _cisco_b64_decode(parts[3])
        iterations = IOS8_DEFAULT_ITER  # Cisco fixed
...
# ---------- Validation ----------
def validate_password(pw, minlen, maxlen):
    if pw is None:
...
def read_password_noninteractive(args):
    # Priority: -pwd > -env VAR > piped stdin
    ...
    if not sys.stdin.isatty():
        data = sys.stdin.read()
        if data.endswith("\n"):
            data = data[:-1]  # strip one trailing newline
        return data
    return None

# ---------- main ----------
def main():
    # Decide color use before constructing argparse (TTY + not explicitly disabled)
    ...
    ap.add_argument("-verify", metavar="HASH", help="Verify a password against an existing hash.")
    ...
    # Respect -no-color after parsing too
    if args.no_color:
        USE_COLOR = False

    # Startup banner (generation mode)
    if not args.quiet and not args.verify:
        print(colorize("Cisco HashGen v1.1 — Generate and verify Cisco-compatible PBKDF2 hashes", "bold", "cyan", use_color=USE_COLOR))
